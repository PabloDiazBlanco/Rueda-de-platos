...
function mealComponents(data, meal) {
  ...
  if (meal.closedDish) {
    ...
    // Ajuste fino de grasa: si el motor de objetivos ha añadido una grasa para acercarse al objetivo
    // de esa comida, aparece aquí como un componente más (editable igual que el resto).
...
function generateMenu(data, avoid = {}) {
  ...
  // Para cada Comida/Cena normal (proteína + carbo + verdura): primero se calcula cuánto carbohidrato
  // y proteína ya aportan "de fondo" la verdura y los garbanzos (que no se tocan, se quedan en su ración
  // habitual), y se descuenta esa parte antes de resolver la ración de carbo y de proteína — así no se
  // cuenta dos veces lo que ya aporta el resto del plato. La grasa queda como resultado de todo lo anterior,
  // y si se queda corta respecto al objetivo, se añade aceite de oliva como ajuste fino visible.
  // Los platos cerrados, desayunos y meriendas no se tocan por ahora.
  if (data.objetivos) {
    ...
    const grasas = data.ingredients.filter((i) => i.category === "grasa" && i.active !== false && i.foodId);
    ...
    slots
      ...
      .forEach((s) => {
        ...
        const verduraIng = s.verdura ? data.ingredients.find((i) => i.name === s.verdura) : null;
        ...
        const verduraFood = verduraIng ? getFood(data, verduraIng.foodId) : null;
        ...
        // Carbohidratos: se descuenta lo que ya aportan verdura y garbanzos a su ración habitual (1).
        const carbVerdura = verduraFood ? (verduraFood.carb * (verduraIng.gramos || 0)) / 100 : 0;
        const carbGarbanzos = garbanzosFood ? (garbanzosFood.carb * (garbanzosIng.gramos || 0) * 0.5) / 100 : 0;
        const carbObjetivoParaCarbo = Math.max(0, objetivoComida.carb - carbVerdura - carbGarbanzos);
        ...
        // Proteína: se descuenta lo que ya aportan verdura, garbanzos, y el carbo (con su ración ya resuelta arriba).
        const racionCarboResuelta = nuevasRaciones.carbo !== undefined ? nuevasRaciones.carbo : 1;
        const protVerdura = verduraFood ? (verduraFood.prot * (verduraIng.gramos || 0)) / 100 : 0;
        ...
        const protObjetivoParaProteina = Math.max(0, objetivoComida.prot - protVerdura - protGarbanzos - protCarbo);
        ...
        // Grasa como resultado de lo anterior; si falta para llegar al objetivo, se rellena con una grasa.
        const actual = mealTotals(data, s);
        const faltaGrasa = objetivoComida.fat - actual.fat;
        if (faltaGrasa > 1) {
          if (grasas.length) {
            const elegida = weightedPick(grasas, "probabilidad");
            const elegidaFood = getFood(data, elegida.foodId);
            if (elegidaFood && elegidaFood.fat > 0 && elegida.gramos) {
              const gramosNecesarios = Math.min(300, Math.round((faltaGrasa / elegidaFood.fat) * 100));
              s.grasaExtra = { nombre: elegida.name, foodId: elegida.foodId, gramos: gramosNecesarios };
            }
          } else if (aove) {
            s.aceiteExtra = Math.round(faltaGrasa);
          }
        }
      });
  }
...
const SPECIAL_TABS = { menu: MENU_TAB_META, alimentos: FOODS_TAB_META, combos: RULES_TAB_META, perfil: PROFILE_TAB_META };

function TabBar({ tab, setTab }) {
  const tabs = ["menu", "proteina", "carbo", "verdura", "grasa", "desayuno", "merienda", "cerrado", "especial", "combos", "alimentos", "perfil"];
  return (
    <nav
      style={{
        display: "flex",
